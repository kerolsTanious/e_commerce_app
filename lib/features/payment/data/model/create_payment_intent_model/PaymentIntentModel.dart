...
/// id : "pi_3R1uuADwUpql7Any0ZlZLbyd"
...
/// client_secret : "pi_3R1uuADwUpql7Any0ZlZLbyd_secret_JO5AUJDdkTXDDl8oTBj8pStSf"
...
/// on_behalf_of : null
/// payment_method : null
/// payment_method_configuration_details : {"id":"pmc_1R1qP8DwUpql7AnyPKnqqISl","parent":null}
...
class PaymentIntentModel {
  ...
  num? get amountCapturable => _amountCapturable ?? 0;

  AmountDetails? get amountDetails => _amountDetails ?? AmountDetails();

  num? get amountReceived => _amountReceived ?? 0;

  dynamic get application => _application ?? dynamic;

  dynamic get applicationFeeAmount => _applicationFeeAmount ?? dynamic;

  AutomaticPaymentMethods? get automaticPaymentMethods =>
      _automaticPaymentMethods ?? AutomaticPaymentMethods();

  dynamic get canceledAt => _canceledAt ?? dynamic;

  dynamic get cancellationReason => _cancellationReason ?? dynamic;
  ...
  dynamic get customer => _customer ?? dynamic;

  dynamic get description => _description ?? dynamic;

  dynamic get invoice => _invoice ?? dynamic;

  dynamic get lastPaymentError => _lastPaymentError ?? dynamic;

  dynamic get latestCharge => _latestCharge ?? dynamic;

  bool? get livemode => _livemode ?? false;

  dynamic get metadata => _metadata ?? dynamic;

  dynamic get nextAction => _nextAction ?? dynamic;

  dynamic get onBehalfOf => _onBehalfOf ?? dynamic;

  dynamic get paymentMethod => _paymentMethod ?? dynamic;

  PaymentMethodConfigurationDetails? get paymentMethodConfigurationDetails =>
      _paymentMethodConfigurationDetails ?? PaymentMethodConfigurationDetails();

  PaymentMethodOptions? get paymentMethodOptions => _paymentMethodOptions ?? PaymentMethodOptions();

  List<String>? get paymentMethodTypes => _paymentMethodTypes ?? [];

  dynamic get processing => _processing ?? dynamic;

  dynamic get receiptEmail => _receiptEmail ?? dynamic;

  dynamic get review => _review ?? dynamic;

  dynamic get setupFutureUsage => _setupFutureUsage ?? dynamic;

  dynamic get shipping => _shipping ?? dynamic;

  dynamic get source => _source ?? dynamic;

  dynamic get statementDescriptor => _statementDescriptor ?? dynamic;

  dynamic get statementDescriptorSuffix => _statementDescriptorSuffix ?? dynamic;

  String? get status => _status ?? "";

  dynamic get transferData => _transferData ?? dynamic;

  dynamic get transferGroup => _transferGroup ?? dynamic;
